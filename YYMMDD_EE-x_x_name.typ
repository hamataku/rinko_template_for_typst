...
= 参照や引用
== 参照
```#label("fig:figure1")```のように参照したい項目に```#label```を付与しておき，```@fig:figure1```のようにすることで参照することができます．図や表，数式，章などに対して参照することができます．

== 引用
bibtexを使用して文献を引用することができます．以下に参考文献を記述しておきます．bibtexはgoogle scholarなどで適宜取得してください．
